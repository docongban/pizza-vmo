...
public class CartServiceImpl implements CartService {
	...
	/*
	 * Tính toán các giá trị discount Version: 1.0
	 */
	...
	public String checkOut(Model model, HttpSession session) {
		...
		if (item_list != null && accountSession != null) {
			...
			List<Discount> discounts = this.discountRepository.getAllSort();
			for(int i=0;i<discounts.size();i++){
				if(total<Math.round(discounts.get(0).getValue()) || total>Math.round(discounts.get(discounts.size()-1).getValue())){
					break;
				}
				if(total>=Math.round(discounts.get(i).getValue()) && total<Math.round(discounts.get(i+1).getValue())){
					totalAfterDiscount = total-Math.round(total*discounts.get(i).getDiscount()/100);
					break;
				}
			}
			if(totalAfterDiscount == 0L){
			...
			orderAccount.setTotal(total);
			orderAccount.setTotalAfterDiscount(totalAfterDiscount);
...
//				orderRepository.save(order);
	...
	public ModelAndView payment(Long amount, HttpSession session) {
		...
		modelAndView.addObject("date", Instant.now());
		return modelAndView;
	}
